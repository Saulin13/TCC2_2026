...
def test_unique_labels_string_labels():
    result = unique_labels(['a', 'b', 'c'], ['b', 'd'])
...
def test_unique_labels_multilabel_sparse_csr():
...
def test_unique_labels_multilabel_sparse_csc():
    y1 = csc_matrix([[1, 0, 1], [0, 1, 0]])
    y2 = csc_matrix([[0, 1, 1], [1, 0, 0]])
    result = unique_labels(y1, y2)
    expected = np.array([0, 1])
    np.testing.assert_array_equal(result, expected)


def test_unique_labels_multilabel_sparse_coo():
    y1 = coo_matrix([[1, 0, 1], [0, 1, 0]])
    y2 = coo_matrix([[0, 1, 1], [1, 0, 0]])
    result = unique_labels(y1, y2)
    expected = np.array([0, 1])
    np.testing.assert_array_equal(result, expected)
...
def test_unique_labels_mixed_string_and_number_raises():
    with pytest.raises(ValueError, match="Mix of label input types"):
        unique_labels([1, 2, 3], ['a', 'b'])


def test_unique_labels_different_multilabel_sizes_raises():
    y1 = np.array([[1, 0, 1], [0, 1, 0]])
    y2 = np.array([[1, 0], [0, 1]])
...
def test_unique_labels_three_arrays():
    result = unique_labels([1, 2], [3, 4], [5, 6])
    expected = np.array([1, 2, 3, 4, 5, 6])
    np.testing.assert_array_equal(result, expected)


def test_unique_labels_with_duplicates_across_arrays():
    result = unique_labels([1, 1, 2], [2, 2, 3], [3, 3, 4])
    expected = np.array([1, 2, 3, 4])
    np.testing.assert_array_equal(result, expected)


def test_unique_labels_float_labels():
    result = unique_labels([1.0, 2.0, 3.0], [2.0, 4.0])

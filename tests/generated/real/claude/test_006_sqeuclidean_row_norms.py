...
def test_sqeuclidean_row_norms_negative_values():
    X = np.array([[-1.0, -2.0, -3.0],
                  [1.0, -1.0, 2.0]], dtype=np.float64, order='C')
    result = sqeuclidean_row_norms(X, num_threads=1)
    expected = np.array([14.0, 6.0], dtype=np.float64)
    np.testing.assert_array_almost_equal(result, expected)
...
def test_sqeuclidean_row_norms_sparse_csr_float64():
...
def test_sqeuclidean_row_norms_sparse_csr_float32():
    X_dense = np.array([[1.0, 0.0, 3.0],
...
def test_sqeuclidean_row_norms_large_values():
    X = np.array([[100.0, 200.0],
                  [300.0, 400.0]], dtype=np.float64, order='C')
    result = sqeuclidean_row_norms(X, num_threads=1)
    expected = np.array([50000.0, 250000.0], dtype=np.float64)
    np.testing.assert_array_almost_equal(result, expected)
...
def test_sqeuclidean_row_norms_invalid_dtype_int64():
    X = np.array([[1, 2, 3],
                  [4, 5, 6]], dtype=np.int64, order='C')
    with pytest.raises(ValueError, match="Only float64 or float32 datasets are supported"):
        sqeuclidean_row_norms(X, num_threads=1)

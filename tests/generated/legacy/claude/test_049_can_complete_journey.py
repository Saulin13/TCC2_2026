...
def test_can_complete_journey_multiple_resets():
    gas_stations = get_gas_stations([1, 2, 3, 4, 5], [2, 3, 4, 1, 1])
    assert can_complete_journey(gas_stations) == 3


def test_can_complete_journey_last_station_start():
    gas_stations = get_gas_stations([1, 1, 1, 10], [2, 2, 2, 1])
    assert can_complete_journey(gas_stations) == 3


def test_can_complete_journey_two_stations_possible():
    gas_stations = get_gas_stations([3, 5], [2, 4])
    assert can_complete_journey(gas_stations) == 0


def test_can_complete_journey_two_stations_impossible():
    gas_stations = get_gas_stations([2, 3], [3, 4])
    assert can_complete_journey(gas_stations) == -1
...
def test_can_complete_journey_large_values():
    gas_stations = get_gas_stations([1000, 2000, 3000], [1500, 1500, 1500])
    assert can_complete_journey(gas_stations) == 0


def test_can_complete_journey_barely_possible():
    gas_stations = get_gas_stations([2, 3, 4], [3, 3, 3])
    assert can_complete_journey(gas_stations) == 0

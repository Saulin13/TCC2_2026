...
def test_majority_vote_multiple_winners():
...
def test_majority_vote_large_votes_needed():
    result = majority_vote([1, 1, 2, 2, 3, 3], 10)
    assert sorted(result) == [1, 2, 3]


def test_majority_vote_exact_threshold():
    result = majority_vote([1, 1, 1, 2, 2, 3], 3)
    assert result == [1]


def test_majority_vote_two_candidates_equal():
    result = majority_vote([1, 1, 2, 2], 2)
    assert result == []


def test_majority_vote_clear_winner():
    result = majority_vote([1, 1, 1, 1, 2, 3], 2)
    assert result == [1]


def test_majority_vote_votes_needed_equals_list_length():
    result = majority_vote([1, 2, 3, 4, 5], 5)
    assert sorted(result) == [1, 2, 3, 4, 5]


def test_majority_vote_negative_numbers():
    result = majority_vote([-1, -1, -1, 0, 0, 1], 3)
    assert result == [-1]


def test_majority_vote_mixed_distribution():
    result = majority_vote([1, 2, 3, 1, 2, 3, 1, 2, 3, 1], 4)
    assert result == [1]


def test_majority_vote_votes_needed_greater_than_unique():
    result = majority_vote([1, 1, 1, 2, 2, 2], 5)
    assert sorted(result) == [1, 2]


def test_majority_vote_zero_division_edge():

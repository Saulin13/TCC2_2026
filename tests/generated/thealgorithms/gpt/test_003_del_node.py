...
def sample_tree():
    ...
    node5 = MyNode(5)
    root = MyNode(4, node2, node5)
    return root

def test_del_node_normal_case(sample_tree):
    # Delete a node with two children
    new_root = del_node(sample_tree, 2)
    assert new_root.get_data() == 4
    assert new_root.get_left().get_data() == 3
    assert new_root.get_left().get_left().get_data() == 1

def test_del_node_leaf(sample_tree):
    # Delete a leaf node
    new_root = del_node(sample_tree, 5)
    assert new_root.get_data() == 4
    assert new_root.get_right() is None

def test_del_node_single_child(sample_tree):
    # Delete a node with a single child
    new_root = del_node(sample_tree, 3)
    assert new_root.get_data() == 4
    assert new_root.get_left().get_data() == 2
    assert new_root.get_left().get_right() is None

def test_del_node_root(sample_tree):
    # Delete the root node
    new_root = del_node(sample_tree, 4)
    assert new_root.get_data() == 3
    assert new_root.get_right().get_data() == 5

def test_del_node_non_existent(sample_tree, capsys):
    # Attempt to delete a non-existent node
    new_root = del_node(sample_tree, 10)
    captured = capsys.readouterr()
    assert captured.out == "No such data\n"
    assert new_root.get_data() == 4
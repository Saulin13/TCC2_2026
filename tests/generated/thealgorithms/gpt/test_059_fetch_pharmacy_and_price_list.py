...
def test_fetch_pharmacy_and_price_list_valid_input():
    # Mock response for a valid input
    mock_html = '''
...
def test_fetch_pharmacy_and_price_list_http_error():
    with patch('httpx.get', side_effect=httpx.HTTPError):
        result = fetch_pharmacy_and_price_list("aspirin", "12345")
        assert result is None

def test_fetch_pharmacy_and_price_list_empty_response():
    # Mock response for an empty HTML page
    mock_response = Mock()
    mock_response.text = ''

    ...
    <html>
        <body>
            ...
            <div class="grid-x pharmCard">
                ...
                <span class="price price-large">$23.00</span>
            </div>
        </body>
    </html>
    """
...
def test_fetch_pharmacy_and_price_list_normal_case(mock_html_response):
    with patch('web_programming.fetch_well_rx_price.httpx.get') as mock_get:
        ...
        assert result[2]["price"] == "$23.00"


def test_fetch_pharmacy_and_price_list_with_valid_inputs(mock_html_response):
    with patch('web_programming.fetch_well_rx_price.httpx.get') as mock_get:
        mock_response = Mock()
        mock_response.text = mock_html_response
        mock_response.raise_for_status.return_value = mock_response
        ...
        result = fetch_pharmacy_and_price_list("lipitor", "90210")
        
        assert isinstance(result, list)
        assert len(result) > 0
        assert all("pharmacy_name" in item for item in result)
...
def test_fetch_pharmacy_and_price_list_no_results(mock_empty_html_response):
    with patch('web_programming.fetch_well_rx_price.httpx.get') as mock_get:
        mock_response = Mock()
        ...
        result = fetch_pharmacy_and_price_list("unknowndrug", "00000")
...
def test_fetch_pharmacy_and_price_list_timeout_error():
    with patch('web_programming.fetch_well_rx_price.httpx.get') as mock_get:
        import httpx
...
def test_fetch_pharmacy_and_price_list_network_error():
    with patch('web_programming.fetch_well_rx_price.httpx.get') as mock_get:
        import httpx

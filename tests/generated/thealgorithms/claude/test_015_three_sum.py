...
def test_three_sum_exact_three_elements_sum_zero():
    result = three_sum([-1, 0, 1])
    assert result == [[-1, 0, 1]]


def test_three_sum_exact_three_elements_no_sum():
    result = three_sum([1, 2, 3])
    assert result == []


def test_three_sum_negative_and_positive():
    result = three_sum([-3, -1, 0, 1, 2, 3])
    expected = [[-3, 0, 3], [-3, 1, 2], [-1, 0, 1]]
    assert sorted(result) == sorted(expected)


def test_three_sum_with_many_duplicates():

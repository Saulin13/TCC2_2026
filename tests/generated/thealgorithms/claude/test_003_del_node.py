...
def test_del_node_from_left_subtree():
    left = create_node(5)
    right = create_node(15)
    root = create_node(10, left=left, right=right)
...
def test_del_node_from_right_subtree():
    left = create_node(5)
    right = create_node(15)
    root = create_node(10, left=left, right=right)
...
def test_del_node_nonexistent_data_left(capsys):
    left = create_node(5)
    right = create_node(15)
    root = create_node(10, left=left, right=right)
    result = del_node(root, 3)
...
def test_del_node_nonexistent_data_right():
    left = create_node(5)
    right = create_node(15)
    root = create_node(10, left=left, right=right)
...
def test_del_node_complex_tree():
    ...
    assert result.get_right().get_left() is not None
    assert result.get_right().get_left().get_data() == 15
...
def test_del_node_triggers_rebalancing():
    ...
    node6 = create_node(6)
    node7 = create_node(7, right=node6)
    root = create_node(5, left=node2, right=node7)
...
def test_del_node_with_string_data():

"""
Avalia testes gerados pelo Claude usando GPT-4o como avaliador.
Reutiliza a mesma lógica e formato do 06_evaluate_tests_llm.py.
"""
...
def build_claude_test_paths(df_sample: pd.DataFrame, tests_dir: Path) -> dict[str, Path]:
    """Mapeia function_name -> arquivo de teste gerado pelo Claude."""
    mapping: dict[str, Path] = {}
    for pos, (_, row) in enumerate(df_sample.iterrows(), start=1):
        func_name = str(row["function_name"])
        mapping[func_name] = tests_dir / f"test_{pos:03d}_{func_name}.py"
    return mapping
...
def evaluate_one(
    ...
    *,
    ...
    if test_path.is_file():
        ...
        test_code = f"(arquivo de teste não encontrado: {test_path})"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Avalia testes gerados pelo Claude usando GPT como avaliador."
    )
...
def main() -> None:
    ...
    tests_dir = cfg.tests_dir("claude")
    input_coverage = cfg.result_csv("cobertura_testes_gerados_claude")
    output_eval = cfg.result_csv("avaliacao_gpt_sobre_testes_claude")

    load_dotenv(BASE_DIR / ".env")

    print(f"Dataset: {cfg.key}")
    print(f"Amostra usada: {sample_file}")
    print(f"Testes Claude: {tests_dir}")
    print(f"Cobertura Claude: {input_coverage}")
    ...
    if not tests_dir.is_dir():
        print(f"Erro: diretório de testes não encontrado: {tests_dir}")
        return
    ...
    n = len(df_selected)
    if n == 0:
        print("Nenhuma linha elegível em cobertura_testes_gerados_claude.csv para avaliar.")
        return

    df_sample = pd.read_csv(sample_file)
    test_paths = build_claude_test_paths(df_sample, tests_dir)
    ...
    df_eval["test_file"] = df_eval["function_name"].map(
        lambda fn: test_paths[str(fn)].relative_to(BASE_DIR).as_posix()
        if str(fn) in test_paths
        else ""
    )
    ...
    print(f"Modelo OpenAI (avaliador): {model}")
    print(f"Iniciando avaliação GPT sobre testes Claude ({n} entradas)...")

    rows: list[dict[str, object]] = []
    for i in range(n):
        row = df_eval.iloc[i].copy()
        name = row["function_name"]
        test_path = test_paths.get(str(name), tests_dir / Path(str(row["test_file"])).name)

        print(f"[{i + 1}/{n}] {name}...")

        try:
            row_out = evaluate_one(client, template, row, model=model, test_path=test_path)
    ...
    print(f"Arquivo salvo: {output_eval}")
